...
all.tdm.95 <- removeSparseTerms(all.tdm, 0.95) 
...
tri.tdm.97 <- removeSparseTerms(tri.tdm, 0.97)
...
count.all$sum <- rowSums(count.all[,c(1:176)])
count.all$word <- row.names(count.all)
count.all <- count.all[,c(179, 177, 1:176, 178)]

#write.csv(count.all, "seinfeld_tdm_episode.csv", row.names=FALSE)
